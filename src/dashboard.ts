...
import { todayInTz, zonedDayStartUtc, zonedNextDayStartUtc } from "./tz.js";
...
function stravaConfig(): { clientId: string; clientSecret: string } | null {
...
/** Same origin the request came in on, e.g. "https://your-app.onrender.com" —
 *  used to build the Strava OAuth redirect_uri so it always matches whatever
 *  domain the dashboard is actually being loaded from, without a separate env
 *  var to keep in sync. */
...
export interface MovementResult {
    /** Whether the owner has completed the "Connect Strava" flow at least
     *  once (a token pair exists), not whether today's fetch succeeded. */
    connected: boolean;
    /** null = connected but today's activities couldn't be loaded right now
     *  (Strava outage, expired grant, etc.) — distinct from [], which means
     *  "loaded fine, nothing logged yet today." */
    activities: StravaActivity[] | null;
}

/**
 * Movement card content: today's activity calories via Strava, informational
 * only — this deliberately never touches the calorie goal or the ring (see
 * the "info only, no adjustment" decision). Returns null when the STRAVA_*
 * env vars aren't set at all, meaning the feature is off for this deploy and
 * the card should not render. A Strava outage or a not-yet-connected account
 * must not take the rest of the dashboard down with it — every failure here
 * degrades to a MovementResult the card can render an explanatory state for,
...
async function getMovement(
    ...
    const store = getStravaTokenStore();
    let tokens;
    try {
        tokens = await store.getTokens();
    } catch (err) {
        console.error("Dashboard: Strava token lookup failed:", err);
        return { connected: false, activities: null };
    }
    if (!tokens) return { connected: false, activities: [] };

    try {
        const dayStartUtc = zonedDayStartUtc(today, tz);
        const dayEndUtc = zonedNextDayStartUtc(today, tz);
        const activities = await getTodaysMovement(
            store,
            config,
            dayStartUtc,
            dayEndUtc,
        ...
        console.error("Dashboard: Strava movement fetch failed:", err);
        return { connected: true, activities: null };
    }
}

/**
 * /strava/authorize and /strava/callback — the one-time "connect Strava"
 * flow for the dashboard's Movement card. Gated by the same DASHBOARD_TOKEN
 * as /dashboard itself (carried through as OAuth `state`, since Strava's
 * redirect only echoes back code/state/scope, not arbitrary query params) —
 * this is a single-owner deploy, so there is no per-user authorization here,
 * just "is this the owner." 404s (not 401/403) on any failure, matching
 * /dashboard's reasoning: a guessed-but-wrong token should look identical to
 * the route not existing.
 */
export function registerStravaRoutes(app: Hono): void {
    app.get("/strava/authorize", (c) => {
        const token = c.req.query("token");
        const expected = process.env.DASHBOARD_TOKEN;
        const config = stravaConfig();
        if (!expected || !token || token !== expected || !config) {
            return c.notFound();
        }
        const redirectUri = `${requestOrigin(c)}/strava/callback`;
        return c.redirect(buildAuthorizeUrl(config, redirectUri, token), 302);
    });
    ...
    app.get("/strava/callback", async (c) => {
        ...
        const config = stravaConfig();
        const expected = process.env.DASHBOARD_TOKEN;
        // state must match DASHBOARD_TOKEN — this is what stops a stranger who
        // finds this public callback URL from completing their own Strava
        // authorization against our token store (see registerStravaRoutes'
        // doc comment). Only a redirect that started at /strava/authorize
        // with the real token produces a matching state.
        if (!code || !config || !expected || state !== expected) {
            return c.notFound();
        }

        try {
            const tokens = await exchangeCodeForTokens(code, config);
            await getStravaTokenStore().saveTokens(tokens);
        } catch (err) {
            console.error("Strava callback failed:", err);
            return c.text("Strava connection failed", 503);
        }
        return c.redirect(`/dashboard?token=${encodeURIComponent(expected)}`, 302);
    });
}
...
export function registerDashboardRoute(app: Hono): void {
    app.get("/dashboard", async (c) => {
        ...
        try {
            ...
            const [meals, goals, latestWeight, movement] = await Promise.all([
...
/** The Movement card body — connect prompt, an explanatory note, or the
 *  activity list + total. Informational only, by design: see MovementResult. */
function movementCard(movement: MovementResult, token: string): string {
    if (!movement.connected) {
        return `<div class="empty">Not connected. <a class="connect-link" href="/strava/authorize?token=${encodeURIComponent(token)}">Connect Strava</a> to see today's activity here.</div>`;
    }
    if (movement.activities === null) {
        return `<div class="empty">Couldn't load today's activity right now.</div>`;
    }
    if (movement.activities.length === 0) {
        return `<div class="empty">No activity logged yet today.</div>`;
    }
    const totalCal = movement.activities.reduce(
        (sum, a) => sum + (a.calories ?? 0),
        0,
    );
    const rows = movement.activities
        .map((a) => {
            const cal = a.calories != null ? `${Math.round(a.calories)} kcal` : "—";
            return `<div class="meal"><div class="meal-time">${a.movingMinutes}m</div><div class="meal-desc">${escapeHtml(a.name)} <span class="row-sub-inline">(${escapeHtml(a.type)})</span></div><div class="meal-kcal">${cal}</div></div>`;
        })
        .join("");
    return `
    <div class="row"><div class="row-label">Burned today</div><div class="row-value">${Math.round(totalCal).toLocaleString()} kcal</div></div>
    ${rows}`;
}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .connect-link { color: #5b9bd9; text-decoration: none; }
...
<body>
  ...
  ${movement ? `<div class="card"><h2>Movement</h2>${movementCard(movement, token)}</div>` : ""}
